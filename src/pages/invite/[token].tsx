          ...
          <div className="space-y-3">
            ...
            {error.toLowerCase().includes("email mismatch") && (
              <div className="space-y-2 text-sm text-gray-700">
                <p>
                  You are signed in as <strong>{session?.user?.email ?? session?.user?.name}</strong>, but this invite was sent to a different email.
                </p>
                <p>Please sign out and sign in with the email address that received this invite.</p>
                <div className="flex items-center justify-center gap-2">
                  <button className="rounded border px-3 py-1" onClick={() => void signOut({ callbackUrl: "/signin" })}>
                    Sign out
                  </button>
