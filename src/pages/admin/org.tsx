...
export default function AdminOrgPage() {
  ...
  const def = { start: new Date(Date.now() - 6 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10), end: new Date().toISOString().slice(0, 10) };
            ...
            <div className="rounded border p-4">
              ...
              <div className="space-y-2">
                {(mgrAgg.data ?? []).map((m: any) => (
                  <a key={m.managerId} className="flex items-center justify-between text-sm hover:underline" href={`/manager?mid=${m.managerId}`}>
                    <span className="truncate">{m.managerName}</span>
                    <span className="text-gray-600">users: {m.usersCount} · entries: {m.entriesCount}</span>
                  </a>
                ))}
                {(mgrAgg.data ?? []).length === 0 && <div className="text-sm text-gray-500">No data</div>}
              </div>
            </div>
            <div className="rounded border p-4">
              ...
              <div className="space-y-2">
                ...
                {(usrAgg.data ?? []).length === 0 && <div className="text-sm text-gray-500">No data</div>}
              </div>
            </div>
          </section>
        )}
        {!state.data?.org && <p className="text-sm text-gray-600">No organization found. Create one at /onboarding.</p>}
        {state.data?.org && (
                  ...
                  <tr key={m.id}>
                    ...
                    <td className="border-b p-3">
                      <select
                        ...
                        value={m.role}
                        onChange={(e) => setRole.mutate({ membershipId: m.id, role: e.target.value as any })}
                      >
                    ...
                    <td className="border-b p-3">
                      {m.role === "USER" ? (
                        <select
                          className="rounded border p-1"
                          value={m.managerId ?? ""}
                          onChange={(e) => assign.mutate({ membershipId: m.id, managerMembershipId: e.target.value || null })}
                        >
        ...
        {state.data?.org && (
          <div className="mt-6 rounded border p-4">
            ...
            <div className="mb-3 flex flex-wrap items-center gap-2">
              ...
              />
              <select className="rounded border p-2" value={emailRole} onChange={(e) => setEmailRole(e.target.value)}>
                <option value="ADMIN">ADMIN</option>
                <option value="MANAGER">MANAGER</option>
                <option value="USER">USER</option>
              </select>
              ...
              <button
                ...
                onClick={async () => {
                  const res = await assignByEmail.mutateAsync({ email, role: emailRole as any, managerMembershipId: emailManager || null });
