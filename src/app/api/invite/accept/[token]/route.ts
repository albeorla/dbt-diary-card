...
export async function GET(_: Request, context: { params: Promise<{ token: string }> }) {
  ...
  if (!token) return NextResponse.redirect(new URL("/signin", env.NEXTAUTH_URL || "http://localhost:3000"));
  ...
  await db.orgMembership.upsert({
    ...
    create: { orgId: invite.orgId, userId: user.id, role: invite.role, managerId: invite.managerId ?? undefined },
  });
  ...
  await db.orgInvite.update({ where: { id: invite.id }, data: { consumedAt: new Date(), consumedBy: user.id } });
