  ...
  upsert: protectedProcedure
    .input(
      z.object({
        ...
        emotions: z.array(z.object({ emotion: z.string(), rating: z.number().int().min(0).max(10) })),
        urges: z.array(
          z.object({
            urgeType: z.string(),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (emotions.length) {
        await ctx.db.emotionRating.createMany({
          data: emotions.map((e) => ({ entryId: entry.id, emotion: e.emotion as any, rating: e.rating })),
        });
      }

      if (urges.length) {
        await ctx.db.urgeBehavior.createMany({
          data: urges.map((u) => ({ entryId: entry.id, urgeType: u.urgeType as any, intensity: u.intensity, actedOn: u.actedOn })),
        });
      }

      if (skills.length) {
        const skillRecords = await ctx.db.dBTSkill.findMany({ where: { name: { in: skills } }, select: { id: true } });
        await ctx.db.skillUsed.createMany({ data: skillRecords.map((s) => ({ entryId: entry.id, skillId: s.id })) });
      }
  ...
  getByDate: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return ctx.db.diaryEntry.findFirst({
        ...
        include: { emotionRatings: true, urgesBehaviors: true, skillsUsed: { include: { skill: true } } },
      });
    }),
