...
export const orgRouter = createTRPCRouter({
  state: protectedProcedure.query(async ({ ctx }) => {
    const org = ctx.orgId
      ? await ctx.db.organization.findUnique({ where: { id: ctx.orgId } })
      : null;
    let membershipId: string | null = null;
    if (ctx.orgId && ctx.session?.user?.id) {
      const m = await ctx.db.orgMembership.findFirst({
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (ctx.session?.user?.id) {
        await ctx.db.orgMembership.create({
          data: { orgId: org.id, userId: ctx.session.user.id, role: 'ADMIN' },
        });
      }
      return org;
    }),
  ...
  assignManager: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return ctx.db.orgMembership.update({
        where: { id: input.membershipId },
        data: { managerId: input.managerMembershipId ?? undefined },
      });
    }),
  ...
  managerSummary: managerOrAdminProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const entries = await ctx.db.diaryEntry.findMany({
        where: {
          userId: { in: userIds },
          entryDate: { gte: new Date(input.start), lte: new Date(input.end) },
        },
        select: { userId: true },
      });
      ...
      await ctx.db.$transaction([
        ctx.db.orgMembership.upsert({
          ...
          create: {
            ...
            role: inv.role,
            managerId: inv.managerId ?? undefined,
          },
        }),
        ctx.db.orgInvite.update({
          where: { id: inv.id },
          data: { consumedAt: new Date(), consumedBy: ctx.session.user.id },
        }),
      ]);
      return { ok: true };
    }),
  ...
  adminManagerSummary: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      for (const m of managers) {
        ...
        let entriesCount = 0;
        if (userIds.length) {
          entriesCount = await ctx.db.diaryEntry.count({
            where: {
              userId: { in: userIds },
              entryDate: { gte: new Date(input.start), lte: new Date(input.end) },
            },
        ...
        result.push({
          managerId: m.id,
          managerName: m.user?.name ?? m.user?.email ?? m.id,
  ...
  adminUserSummary: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      if (!ctx.orgId) return [];
      const users = await ctx.db.orgMembership.findMany({
        where: { orgId: ctx.orgId, role: 'USER' },
        include: { user: true },
      });
      const result: { userId: string; name: string; entryCount: number }[] = [];
      for (const u of users) {
        const entryCount = await ctx.db.diaryEntry.count({
          where: {
            userId: u.userId,
            entryDate: { gte: new Date(input.start), lte: new Date(input.end) },
          },
  ...
  userEntriesAndLast: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const target = await ctx.db.orgMembership.findFirst({
      ...
      const [count, last] = await Promise.all([
        ctx.db.diaryEntry.count({
          where: {
            userId: input.userId,
            entryDate: { gte: new Date(input.start), lte: new Date(input.end) },
          },
        }),
        ctx.db.diaryEntry.findFirst({
  ...
  userEntryById: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const target = await ctx.db.orgMembership.findFirst({
        where: { orgId: ctx.orgId, userId: entry.userId },
        select: { managerId: true },
      });
  ...
  adminTrendsEmotions: adminProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return Array.from(sums, ([emotion, v]) => ({ emotion, avg: v.n ? v.sum / v.n : 0 })).sort(
        (a, b) => b.avg - a.avg,
      );
    }),
  ...
  managerTrendsEmotions: managerOrAdminProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (!managerId) return [];
      const users = await ctx.db.orgMembership.findMany({
        where: { orgId: ctx.orgId, managerId, role: 'USER' },
        select: { userId: true },
      });
      const userIds = users.map((u) => u.userId);
      if (!userIds.length) return [];
      ...
      return Array.from(sums, ([emotion, avg]) => ({
        emotion,
        avg: avg.n ? avg.sum / avg.n : 0,
