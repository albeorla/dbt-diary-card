...
export function SkillsCheckList({
  ...
  return (
    <section className="mb-8">
      <Typography variant="h6" sx={{ mb: 1 }}>Skills Used</Typography>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        {Object.entries(groupedSkills).map(([module, skills]) => (
          <Paper key={module} variant="outlined" sx={{ p: 2 }}>
            <Box sx={{ mb: 1, display: 'flex', alignItems: 'center', gap: 0.5 }}>
              <Typography variant="subtitle2" sx={{ fontWeight: 600 }}>{SKILL_MODULE_LABELS[module as string] ?? module}</Typography>
