...
export function CalendarView({ year, month }: { year: number; month: number }) {
  ...
  const { data } = api.diary.getRange.useQuery({ startDate: toYMD(startOfGrid), endDate: toYMD(endOfGrid) });
  const entrySet = useMemo(() => new Set((data ?? []).map((e: any) => toYMD(new Date(e.entryDate)))), [data]);
    ...
    <div className="w-full">
      ...
      <div
        ...
        onKeyDown={(e) => {
          ...
          if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setOpenDate(focusDate); }
          if (e.key.toLowerCase() === "t") { e.preventDefault(); const t = toYMD(new Date()); setFocusDate(t); setOpenDate(t); }
        }}
        tabIndex={0}
        ...
        {range(totalDays).map((i) => {
          ...
          return (
            <div key={i} role="gridcell" aria-selected={focusDate === ymd}>
              ...
              hasEntry={hasEntry}
              isToday={isToday}
              onClick={() => { setFocusDate(ymd); setOpenDate(ymd); }}
            />
            </div>
          );
        })}
